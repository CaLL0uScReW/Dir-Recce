...
# TOOL PATH
ABS_PATH = os.path.dirname(os.path.realpath(__file__)).split("recce")[0]
TOOL_PATH = list(
    pathlib.Path(os.path.join(os.path.join(ABS_PATH, "recce"), "")).parents
)[1]
...
\033[93m
Dir-Recce : Advanced Web Application Directory Reconassaince Tool 
\033[91m
                                            -Scripted with <3 by Shivanshu Sharma
"""
...
kwargs = {
    ...
    "delay": None,
    "output": None,
    "wordlist": TOOL_PATH.joinpath(pathlib.Path("wordlists").joinpath("dict.txt")),
    "exts": None,
    "threads": 1,
    "recursive": False,
    "host": None,
...
\t-P --proxy\tUse a proxy to connect to the target
\t-I --ignore\tIgnore words in wordlist, should use regex (e.g: "^test[a-zA-Z0-9]*\.zip")
\t-m --methods\tCheck other methods

\t-O --output\tOutput support json and txt format (e.g: out.json)
\t-C --length\tShow only if response length is ">","<" or "=" (e.g: ">1024")
\t-o --only\tShow only status code, To beseparated by comma (e.g: 200,302)
\t-t --threads\tMax number of concurrent HTTP requests
\t-T --timeout\tSeconds to wait before the timeout connection
\t-E --recursive\tBruteforce recursively
\t-D --dir-rec\tSpecify dir bruteforce recursively
\t-S --sub-dir\tSpecify sub-dir bruteforce
    ...
    if r"%%" in _url:
        ...
        if path_end:
            return (
                _url.split(
                    re.search(
                        r"\/([a-zA-Z0-9_-]*\.[a-zA-Z0-9_-]*)", parse.path, re.I
                    ).group(0)[1:]
                )[0],
                parse.netloc,
            )
