    ...
    def http(self, word, method="GET"):
        ...
        if self.byhostname:
            ...
            try:
                ...
                self.printWarn(
                    "CONNECTION ERROR: Temporary failure in name resolution", True
                )
        # --
        if self.ip != None:
            self.url = re.sub(
                self.host, self.ip + (":" + str(port) if port != None else ""), self.url
            )
        # --
        response = None
        try:
...
def randomData():
    return "{0}={1}&{2}={3}".format(rstring(5), rstring(10), rstring(5), rstring(10))
